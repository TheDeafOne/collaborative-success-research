...
\author[1,*]{Keegan Woodburn}
\author[1]{Caleb Frey}
\author[2]{Chad Peterson}
\author[1]{Ian McCulloh}

\affil[1]{Department of Computer Science, Johns Hopkins University, Baltimore, Maryland, USA}
\affil[2]{HOPE-20} % replace with full institution name if possible

% Corresponding author
\affil[*]{Correspondence to: kwoodbu1@jhu.edu.\\
Other emails: frey7@jhu.edu, imccull4@jhu.edu, chad\_petterson@yahoo.com}

% Additional author emails (non-corresponding)
% \affil[]{frey7@jhu.edu}
% \affil[]{imccull4@jhu.edu}
% \affil[]{chad\_petterson@yahoo.com}
...
Building on this foundation, we develop a predictive modeling framework which combines traditional network metrics, ERGM-informed features, and learned graph embeddings to estimate long-term success, measured by Spotify follower count as of December 2025.
...
Across all models, triadic closure effects are positive. The inclusion of \texttt{gwesp} terms substantially improves model fit relative to a baseline density model, indicating that collaboration ties are more likely to form among artists who already share common collaborators. These effects remain stable across specifications that incorporate attribute-based matching and opportunity controls, indicating that clustering is a robust feature of early-career collaboration networks.
...
\begin{table}[htbp]
\centering
\caption{Five-fold cross-validated performance on the \emph{training} set for CatBoost models across feature sets (training artists $n=68{,}972$; held-out fold $n=13{,}794$--$13{,}795$; corresponding fitting fold $n=55{,}177$--$55{,}178$). Table entries are mean and s.d. across folds. These comparisons are descriptive; no hypothesis test was applied to the cross-validation differences.}
...
Finally, to assess generalization, we trained a \texttt{base+graph+n2v} CatBoost model on the full training set using the selected hyperparameters and evaluated it once on the untouched 20\% test set ($n=17{,}244$). On the test set, the model achieves a log-RMSE of 2.38 and a log-$R^2$ of 0.42. Because the test set is evaluated only once, we report single-point test metrics rather than a cross-validated mean and standard deviation. The close agreement between the training CV estimates and the held-out test performance indicates that the observed gains generalize to unseen artists, directly answering RQ2 in predictive rather than hypothesis-testing terms.
...
\begin{table}[t]
\centering
\caption{Top fifteen features by CatBoost feature importance for the full \texttt{base+graph+n2v} model trained on the full training split ($n=68{,}972$ artists). Feature importance values are descriptive model summaries, not inferential test statistics.}
...
We further examine model behavior using SHAP values, summarized in Figure~\ref{fig:shap}. The SHAP analysis corroborates the global importance rankings and highlights additional network-related features, including the GWESP score and community-participation measure. Together, these results show that long-term success is associated with brokerage positions, cross-community engagement, and deviations from expected collaboration patterns.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/shap.png}
    \caption{SHAP summary plot for the full \texttt{base+graph+n2v} CatBoost model, showing the relative contribution of the most influential features to predicted follower counts. SHAP values were computed on a random sample of $n=300$ held-out test artists. Grey points indicate a categorical variable.}
    \label{fig:shap}
\end{figure}
...
\subsection*{From Network Closure to Structured Advantage}

The introduction framed the contemporary music industry as one in which opportunities circulate through dense, overlapping collaboration networks rather than emerging solely through open, exploratory partner search. The network formation results support a more nuanced version of that claim. The dominance of triadic closure indicates that early-career collaborations arise primarily within tightly connected local neighborhoods rather than through structurally novel partnerships. New ties are therefore embedded in existing collaboration contexts, reinforcing cohesive team structures over time.
...
In sum, our findings support the core premise articulated in the introduction: early-career collaboration networks in the music industry are shaped by structured advantage, network closure, and uneven access to brokerage opportunities rather than by open, uniformly accessible partner search. Within this environment, rare but structurally distinct bridging ties and brokerage positions carry disproportionate predictive value for which artists reach larger audiences. Understanding these patterns provides a network-based lens on how creative careers develop and which forms of connectivity are most closely linked to long-term success.

\section*{Methods}

\subsection*{Statistical Reporting}
Unless otherwise noted, inferential statistics in this manuscript are limited to the ERGM coefficient tables. For those models, all reported $P$ values are numeric two-sided Wald probabilities computed from the coefficient estimate and its reported MPLE standard error, with $\alpha = 0.05$. Sample sizes for each analysis are given in the text, tables, or figure legends. Because the ERGMs were fit by maximum pseudolikelihood rather than MCMC-MLE, the corresponding Wald tests rely on the asymptotic normal approximation for the coefficient estimates rather than on normality of the raw data. No separate normality-based tests on the raw variables were used. The predictive-model comparisons, feature-importance rankings, and SHAP summaries are descriptive model-evaluation results and are therefore reported without hypothesis-test $P$ values.

The ERGM coefficient tables contain multiple simultaneous coefficient tests. We therefore treat those coefficient-level $P$ values as exploratory summaries of the fitted model rather than as a family of confirmatory claims, and we do not apply a multiplicity correction. Interpretation instead emphasizes effect sizes, sign consistency across the model ladder, and changes in pseudo-deviance/AIC/BIC.
...
We begin by constructing the network from the 2.7 million artists available in the MusicBrainz database. Because our predictive task requires a success metric, we restrict our sample to artists who have a valid Spotify identifier and therefore a measurable follower count \cite{SpotifyAPI}. This reduces the total number of musicians we analyze to 352,197. This ensures that all included artists have observable long-term outcomes. We further retain only those artists who have participated in at least one collaborative work during their first five years, since collaboration is the basis for constructing the network that follows, resulting in 86,762 focal artists. Together, these criteria yield a quality, collaboration-relevant population with well-defined early-career periods and consistent success measurements.
...
All ERGMs were fit using the R programming language, with the \texttt{statnet}/\texttt{ergm} packages (v2019.6 and v4.10.1 respectively). The ERGM fitting script begins from the 86,762 focal artists with observed artist-level attributes and augments them with 156,471 collaborator-only placeholder vertices found in the edge list, yielding an analysis graph with $n=243{,}233$ vertices, 1,426,829 observed edges, and 29 billion possible dyads. Because this artist--artist network is extremely large, we estimate each specification using maximum pseudolikelihood estimation (MPLE). MPLE scales to large networks by fitting the dyadwise logistic pseudolikelihood rather than requiring repeated MCMC simulation of full networks during estimation.
...
\end{equation}
When $\deg(i)=0$, we set $\text{weak\_tie\_frac}(i)=0$.
...
We used the Python \texttt{node2vec} implementation with \texttt{networkx} graphs. All node2vec hyperparameters were fixed a priori to commonly used defaults and were not further tuned. The resulting 32-dimensional vectors
$(n2v_0, \dots, n2v_{31})$
...
All network features are computed as described above and then combined with the non-network covariates for regression. Features with heavy right tails (e.g., degree and component size) are included both in raw form and, where noted, in $\log(1+x)$ form.
...
We first perform a preliminary hyperparameter-selection step on the training set with all features. Using 5-fold cross-validation, we identify a single CatBoost hyperparameter configuration and then hold those hyperparameters fixed for the remainder of the analysis. We then compare feature sets using 5-fold cross-validation on the training artists: for each fold, a CatBoost model is trained on four-fifths of the data and evaluated on the remaining fifth using log-RMSE and log-\(R^2\). We report the mean and standard deviation of these metrics across folds in Table~1 to summarize generalization performance across feature sets under a common, fixed set of model hyperparameters.

After selecting the best-performing feature set, we train a final CatBoost model on the full training dataset using the fixed hyperparameters and report its performance once on the held-out test set.


\section*{Data availability}
MusicBrainz data used in this study are publicly accessible at \url{https://musicbrainz.org/doc/MusicBrainz_Database}. Spotify metadata were obtained via the Spotify Web API: https://developer.spotify.com/documentation/web-api, in accordance with its terms of use.
...
Ni, Y., Santos-Rodríguez, R., McVicar, M. \& De Bie, T. 
Hit song science once again a science? (2011).

\bibitem{Zangerle2019HitSongPrediction}
Zangerle, E., Huber, R. \& Yang, Y.-H. M. 
Hit song prediction: leveraging low- and high-level audio features. 
In \textit{Proceedings of the 20th International Society for Music Information Retrieval Conference}, 319--326 (2019).

\bibitem{Choudhary2021Multimodal}
Choudhary, Y. \& Rao, P. 
Literature survey: multimodal prediction for music popularity. 
Technical report, IIT Bombay (2021).

\bibitem{Granovetter1973WeakTies}
Granovetter, M. S. 
The strength of weak ties. 
\textit{American Journal of Sociology} \textbf{78}, 1360--1380 (1973).
...
Available at: \url{https://musicbrainz.org/}

\bibitem{SpotifyAPI}
Spotify. 
Spotify Web API. 
Available at: \url{https://developer.spotify.com/documentation/web-api}

\bibitem{Robins2007IntroERGM}
Robins, G., Pattison, P., Kalish, Y. \& Lusher, D. 
An introduction to exponential random graph (p*) models for social networks. 
\textit{Social Networks} \textbf{29}, 173--191 (2007).
...
Kang, I., Mandulak, M. \& Szymanski, B. K. 
Analyzing and predicting success of professional musicians. 
\textit{Scientific Reports} \textbf{12}, 21838 (2022).

\bibitem{Silva2019SAC}
Silva, M. O., Rocha, L. M. \& Moro, M. M. 
Collaboration profiles and their impact on musical success. 
In \textit{Proceedings of the 34th ACM/SIGAPP Symposium on Applied Computing}, 2070--2077 (2019).

\bibitem{Oliveira2020ISMIR}
Oliveira, G. P., Silva, M. O., Seufitelli, D. B., Lacerda, A. \& Moro, M. M. 
Detecting collaboration profiles in success-based music genre networks. 
In \textit{Proceedings of the 21st International Society for Music Information Retrieval Conference}, 726--732 (2020).

\bibitem{Oliveira2021Thesis}
Oliveira, G. P. 
Analyses of musical success based on time, genre and collaboration. 
M.S. thesis, Universidade Federal de Minas Gerais (2021).

\bibitem{Silva2019WebMedia}
Silva, M. O. \& Moro, M. M. 
Causality analysis between collaboration profiles and musical success. 
In \textit{Proceedings of the Brazilian Symposium on Multimedia and the Web}, 369--376 (2019).

\bibitem{MartinezAlbeniz2023MusicIndustry}
Deshmane, A. \& Martínez-de-Albéniz, V. 
Come together, right now? An empirical study of collaborations in the music industry. 
\textit{Management Science} \textbf{69}, 7217--7235 (2023).

\bibitem{prokhorenkova2018catboost}
Prokhorenkova, L. et al. 
CatBoost: unbiased boosting with categorical features. 
In \textit{Advances in Neural Information Processing Systems} \textbf{31}, 6639--6649 (2018).

\bibitem{Grover2016Node2Vec}
Grover, A. \& Leskovec, J. 
node2vec: scalable feature learning for networks. 
In \textit{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, 855--864 (2016).

\bibitem{Chen2016XGBoost}
Chen, T. \& Guestrin, C. 
XGBoost: a scalable tree boosting system. 
In \textit{Proceedings of the 22nd ACM SIGKDD International Conference on Knowledge Discovery and Data Mining}, 785--794 (2016).
...
\textbf{Figure 1.} SHAP summary plot for the full \texttt{base+graph+n2v} CatBoost model, showing the relative contribution of the most influential features to predicted follower counts. SHAP values were computed on a random sample of $n=300$ held-out test artists. Grey points indicate a categorical variable.
...
C.P. and I.M. conceived the study. C.F. and K.W. conducted the analyses. C.P. provided industry/domain knowledge and expertise. C.F., K.W., and I.M. interpreted the results and contributed to the writing of the manuscript.  All authors reviewed and approved the final manuscript. 
